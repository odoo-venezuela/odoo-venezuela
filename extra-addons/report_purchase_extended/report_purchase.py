...
class report_purchase_ext_order_product(osv.osv):
    _name = "report.purchase.ext.order.product"
    ...
    _columns = {
        ...
        'price_unit': fields.float('Average Price', readonly=True),
        'count': fields.integer('# of Lines', readonly=True),
    }
    _order = 'name desc,price_total desc'
    def init(self, cr):
        cr.execute("""
            create or replace view report_purchase_ext_order_product as (
                select
                    min(l.id) as id,
                    ...
                    (sum(l.product_qty*l.price_unit)/sum(l.product_qty*u.factor))::decimal(16,2) as price_unit
        ...
        """)
report_purchase_ext_order_product()

class report_purchase_ext_order_category(osv.osv):
    _name = "report.purchase.ext.order.category"
    ...
    def init(self, cr):
        cr.execute("""
            create or replace view report_purchase_ext_order_category as (
                select
                    min(l.id) as id,
                    to_char(s.date_order, 'YYYY-MM-01') as name,
                    s.state,
        ...
        """)
report_purchase_ext_order_category()
