...
class account_wh_iva(osv.osv):
    ...
    def check_wh_lines_fortnights(self, cr, uid, ids, context=None):
        ...
        context = context or {}
        per_obj = self.pool.get('account.period')
        error_fn_str = {'True': ' - Second Fortnight)',
                   ...
                   awil_fortnight != eval(awi_brw.fortnight):
                    raise osv.except_osv(_('Invalid action !'),
                        _("Can't compute the taxes of the withholding document."
                          ...
                          " * Withholding VAT document"
                          " correspond to (" + awi_brw.period_id.name +
                          error_fn_str[awi_brw.fortnight] + ".\n"
                          " * Line '" + awil_brw.invoice_id.number + "' belongs "
                          "to (" + per_obj.browse(cr, uid, awil_period_id, context=context).name
                          + error_fn_str[str(awil_fortnight)] +".\n"
                          ))
                    return False
        return True
