...
class report_sxw_ext(report_sxw):
    ...
    def validate_report(self,cr, uid, report_xml_id, id_obj, context):
        pool = pooler.get_pool(cr.dbname)
        print_report_obj =pool.get('print.model.reports')
        report_id = print_report_obj.search(cr,uid,[('report_id','=',report_xml_id)])
        if report_id:
            res = {}
            brw = print_report_obj.browse(cr, uid, report_id[0],context)
            
            
            #~ code= brw.python_code
            #~ localdict = {'x':5}
            #~ exec code in localdict
            #~ exec code
            #~ f(5)
            #~ amount = localdict['result']
            #~ print 'LO QUE DIOOOOOOOOOOO:::',amount
            
            
            res.update({'brw':brw})
            if not brw.allow_repeat:
    ...
    def _default_company(self, cr, uid, context={}):
        ...
        company_obj =pool.get('res.company')
        company_id =  company_obj.search(cr, uid, [('parent_id', '=', False)])[0]
        if company_obj:
...
def new_register_all(db):
    ...
    that registers 'automagically' OpenOopenoffice_reportffice reports.
    """
    ...
    #
    cr = db.cursor()
    cr.execute("SELECT ir.report_name,ir.model,ir.report_rml FROM ir_act_report_xml ir, print_model_reports pm WHERE ir.id = pm.report_id")
    records = cr.dictfetchall()
    cr.close()
    for record in records:
        register_report_sxw_ext( record['report_name'], record['model'],record['report_rml'])
    return value
