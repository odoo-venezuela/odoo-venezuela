...
class commission_payment(osv.osv):
    ...
    _columns = {
        ...
        'total_comm':fields.float('Total a Pagar', digits=(16,2), readonly= True,states={'write':[('readonly',False)]}),
    ...
    def _prepare(self, cr, user, ids, context={}):
        """
        ...
        for commission in commissions:
            ...
            self._unlink(cr, user, ids, context=None)            
            ...
            if len(voucher_ids) == 0:
                ...
                voucher_ids = vouchers.search(cr, user, [('state', '=', 'posted'), ('type', '=', 'bank_rec_voucher'),('date', '>=', date_start),('date', '<=', date_stop)])
                
                commission.write({
                                'voucher_ids': [(6, commission.id, len(voucher_ids) > 0 and voucher_ids or False)],
                                },context=None) 
            
            for vid in voucher_ids:
                    pay = vouchers.read(cr,user,vid,['name', 'date', 'amount', 'payment_ids','move_ids'],context=None)
                    
                    if len(pay['move_ids'])!=0 and len(pay['payment_ids'])!=0:
                        ...
                        for pid in pay['payment_ids']:
                            pay_line_vendor = payments.browse(cr, user, pid, context=None).partner_id.user_id and payments.browse(cr, user, pid, context=None).partner_id.user_id.id or False
                                ...
                                if (pay_line['invoice_id']!=False and pay_line['paid_comm']==False):
                                    ...
                                    if abs(inv['amount_total'] - pay_line['amount'])<= 1.0:
                                        ...
                                        for line in lines_ret_iva:
                                            perc_Ret_IVA = ret_iva_lines.browse(cr, user, line, context=None).retention_rate
                                        no_ret_iva = False
                                    ...
                                    if no_ret_im == True and ret_im_lines.search(cr, user, [('invoice_id', '=', inv_id)]):
                                        ...
                                        for line in lines_ret_im:
                                            perc_Ret_IM += ret_im_lines.browse(cr, user, line, context=None).retencion_munici
                                        no_ret_im = False
                                            ...
                                            if inv_lin['product_id'] != False:
                                                ...
                                                if no_price == False:
                                                    ...
                                                    comm_line_ids.create(cr, user,{
                                                        ...
                                                        'perc_IVA' : perc_IVA,
                                                        'dcto_item': round(dcto,2), 
                                                        'baremo_dcto': bar_dcto_disc,
                                                        'baremo_days': bar_day,
    ...
    def _pre_process(self, cr, user, ids, context={}):
        commissions = self.browse(cr, user, ids, context=None)
        for commission in commissions:
            self._prepare(cr, user, ids, context=None)
    ...
    def _delete(self, cr, user, ids, context={}):
        commissions = self.browse(cr, user, ids, context=None)
    ...
    def _unlink(self, cr, user, ids, context={}):
    ...
    def _decide(self, cr, user, ids, context={}):
        commissions = self.browse(cr, user, ids, context=None)
    ...
    def _going_back(self, cr, user, ids, context={}):
        self.write(cr, user, ids, {
                'state': 'open',
            })
...
class commission_lines(osv.osv):
    ...
    _columns = {
        ...
        'pay_off':fields.float('Pago', digits=(16,2)),
        ...
        'pay_inv': fields.float('Abono Fact.', digits=(16,2)),
        
        'inv_date': fields.date('Fecha Doc.'),
        'days':fields.char('Dias', size=8),
        
        'inv_subtotal': fields.float('SubTot. Doc.', digits=(16,2)),
        
        'item': fields.char('Item', size=256, required=True),
        'price_unit': fields.float('Prec. Unit.', digits=(16,2)),
        'price_subtotal': fields.float('SubTot. Item', digits=(16,2)),
        
        'price_list': fields.float('Precio Lista', digits=(16,2)),
        'price_date': fields.date('Fecha Lista'),
        
        
        'perc_Ret_ISLR': fields.float('Ret ISLR (%)', digits=(16,2)),
        'perc_Ret_IM': fields.float('Ret IM (%)', digits=(16,2)),
        'perc_Ret_IVA': fields.float('Ret IVA (%)', digits=(16,2)),
        'perc_IVA': fields.float('IVA (%)', digits=(16,2)),
        
        'dcto_item':fields.float('Dcto. (%)', digits=(16,2)),
        
        'baremo_dcto':fields.char('Bar. Dcto. (%)', size=16),
        'baremo_days':fields.char('Bar. Dias', size=16),
        'baremo_comm':fields.float('Baremo %Comm.', digits=(16,2)),
        'commission':fields.float('Comm. / Item', digits=(16,2)),
    }
    _defaults = {
        'name': lambda *a: None,
    }
...
class commission_saleman(osv.osv):
    ...
    _columns = {
        ...
        'saleman_name': fields.char('Vendedor', size=256, required=True),
        'saleman_id': fields.many2one('res.users', 'Vendedor', required=True),
        'comm_total':fields.float('Comision a pagar', digits=(16,2)),
...
class commission_invoice(osv.osv):
    ...
    _columns = {
        ...
        'pay_inv': fields.float('Abono Fact.', digits=(16,2)),
        'ret_iva': fields.float('% Ret. IVA', digits=(16,2)),
        'ret_islr': fields.float('% Ret. ISLR', digits=(16,2)),
        'ret_im': fields.float('% Ret. IM', digits=(16,2)),
    }
    _defaults = {
        'name': lambda *a: None,
    }
